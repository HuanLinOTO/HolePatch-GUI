...
class PortTest(object):
    def test_lan(self, addr, source_ip=None, interface=None, info=False):
        ...
        try:
            ...
            Logger.debug(
                "Cannot test port %s from LAN because: %s" % (addr_to_str(addr), ex)
            )
            return 0
        finally:
            sock.close()
...
class ForwardTestServer(object):
    ...
    def start_forward(self, ip, port, toip, toport, udp=False):
        ...
        Logger.debug(
            "fwd-test: Starting test server at %s" % addr_to_uri((ip, port), udp=udp)
        )
        if udp:
...
class ForwardIptables(object):
    ...
    def _iptables_check(self):
        ...
        try:
            output = subprocess.check_output(self.iptables_cmd + ["--version"]).decode()
        ...
        # check nat table
        try:
            subprocess.check_output(self.iptables_cmd + ["-t", "nat", "--list-rules"])
        except (OSError, subprocess.CalledProcessError) as e:
            return False
        return True
    ...
    def _iptables_init(self):
        ...
        subprocess.check_output(self.iptables_cmd + ["-t", "nat", "-N", "NATTER"])
        subprocess.check_output(
        ...
        subprocess.check_output(self.iptables_cmd + ["-t", "nat", "-N", "NATTER_SNAT"])
        subprocess.check_output(
    ...
    def _iptables_clean(self):
        ...
        while self.rules:
            ...
            try:
                ...
                Logger.error(
                    "fwd-iptables: Failed to execute %s: %s" % (ex.cmd, ex.output)
                )
                continue
    ...
    def start_forward(self, ip, port, toip, toport, udp=False):
        ...
        if (ip, port) == (toip, toport):
            raise ValueError(
                "Cannot forward to the same address %s" % addr_to_str((ip, port))
            )
...
class ForwardNftables(object):
    ...
    def _nftables_check(self):
        ...
        try:
            output = subprocess.check_output(self.nftables_cmd + ["--version"]).decode()
        except (OSError, subprocess.CalledProcessError) as e:
            return False
        ...
        # check nat table
        try:
            subprocess.check_output(self.nftables_cmd + ["list table ip nat"])
        except (OSError, subprocess.CalledProcessError) as e:
            return False
        return True
    ...
    def _nftables_init(self):
        ...
        subprocess.check_output(self.nftables_cmd + ["add chain ip nat NATTER"])
        subprocess.check_output(
        ...
        subprocess.check_output(self.nftables_cmd + ["add chain ip nat NATTER_SNAT"])
        subprocess.check_output(
            self.nftables_cmd
            + ["insert rule ip nat PREROUTING counter jump NATTER_SNAT"]
        )
        subprocess.check_output(
    ...
    def _nftables_clean(self):
        ...
        if self.handle > 0:
            subprocess.check_output(
                self.nftables_cmd
                + ["delete rule ip nat NATTER handle %d" % self.handle]
            )
        if self.handle_snat > 0:
            subprocess.check_output(
                self.nftables_cmd
                + ["delete rule ip nat NATTER_SNAT handle %d" % self.handle_snat]
            )
    ...
    def _check_sys_forward_config(self):
        ...
        if os.path.exists(fpath):
            ...
            if buff.strip() != "1":
                raise OSError(
                    "IP forwarding is disabled by system. Please do `sysctl net.ipv4.ip_forward=1`"
                )
        else:
        ...
        while True:
            try:
                ...
                if not closed_socket_ex(ex):
                    Logger.error(
                        "fwd-socket: socket listening thread is exiting: %s" % ex
                    )
                return
            ...
            try:
                ...
                start_daemon_thread(
                    self._socket_tcp_forward, args=(sock_inbound, sock_outbound)
                )
                start_daemon_thread(
                    self._socket_tcp_forward, args=(sock_outbound, sock_inbound)
                )
            except (OSError, socket.error) as ex:
    ...
    def _socket_udp_recvfrom(self):
        ...
        while True:
            try:
                ...
                if not closed_socket_ex(ex):
                    Logger.error(
                        "fwd-socket: socket recvfrom thread is exiting: %s" % ex
                    )
                return
            try:
                if not s:
...
class UPnPClient(object):
    ...
    def _discover(self):
        ...
        for ipaddr, urls in upnp_urls_d.items():
            d = UPnPDevice(
                ipaddr, urls, bind_ip=self._bind_ip, interface=self._bind_interface
            )
            d._load_services()
            devs.append(d)
...
def natter_main(show_title=True):
    ...
    group.add_argument("-U", action="store_true", help="enable UPnP/IGD discovery")
    group.add_argument(
        "-k",
        type=int,
        metavar="<interval>",
    ...
    stun = StunClient(
        stun_srv_list, bind_ip, bind_port, udp=udp_mode, interface=bind_interface
    )
    ...
    if upnp_router:
        ...
        try:
            upnp.forward(
                "", bind_port, bind_ip, bind_port, udp=udp_mode, duration=interval * 3
            )
        except (OSError, socket.error, ValueError) as ex:
    ...
    if not udp_mode:
        ...
        ret3 = port_test.test_lan(
            outer_addr, source_ip=natter_addr[0], interface=bind_interface, info=True
        )
        ...
        if need_recheck:
            ...
            if (
                ...
                or port_test.test_lan(
                    outer_addr, source_ip=natter_addr[0], interface=bind_interface
                )
                == -1
            ):
